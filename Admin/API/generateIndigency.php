<?php
    ...
    $query = "select concat('BI-',RIGHT(count(*)+1000001,5))  from t_indigency_record ";
    ...
    $stmt = $connection->prepare("insert into t_indigency_record (indigency_record_control_number,indigency_record_purpose,indigency_record_remarks,indigency_record_resident_id) values (?,?,?,?)");
    ...
    $query = "select resident_gender,baranggay_officer_position_type,baranggay_officer_id,resident_avatar,term_name,baranggay_officer_position_position,concat(resident_first_name,' ',ifnull(resident_middle_name ,''),' ',resident_last_name) as fullname from r_baranggay_officer as t1 inner join r_resident on resident_id = baranggay_officer_resident_id inner join r_term on baranggay_officer_term_id = term_id inner join r_baranggay_officer_position as t2 on t2.baranggay_officer_position_id = t1.baranggay_officer_position_id where resident_active = 'Active' and baranggay_officer_active = 'Active' and term_active = 'Active' order by concat(resident_last_name,', ',resident_first_name,' ',ifnull(resident_middle_name ,'')) asc";
    $stmt = $connection->prepare($query);
    $stmt->execute();
    $stmt->bind_result($gender,$type,$id,$avatar,$term,$position,$name);
